...
vec3 ads_shading(vec4 lightPos) {
    vec3 s = normalize(lightPos.xyz - v_position.xyz);
    vec3 v = normalize(vec3(-v_position));
    vec3 h = normalize(v + s);

    vec3 ambient = Ka * Intensity;
    vec3 diffuse = Intensity * Kd * max(0.0, dot(s, v_normal.xyz));
    vec3 spec = Intensity * Ks * pow(max(0.0, dot(h, v_normal.xyz)), shininess);
