...
void main() {
    ...
    // Get texture color
    vec4 tcolor = texture2D(u_texture_1, v_texcoord);

    // Noise color
    float r = rand(v_texcoord * sin(u_time));
    float g = rand(v_texcoord * cos(u_time));
    float b = rand(vec2(r, g));
    vec3 noise = vec3(r, g, b);
    float ratio = abs(sin(u_time)) * 0.65;

    // Convert noise
    gl_FragColor = mix(tcolor, vec4(noise, 1.0), ratio) * v_color;
}
